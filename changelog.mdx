...
We're excited to announce the addition of two new routes to the EigenExplorer API, [Retrieve AVS Operators](/api-reference/endpoint/avs/retrieve-all-operators-for-a-given-avs-address.mdx) and [Retrieve AVS Stakers](/api-reference/endpoint/avs/retrieve-all-stakers-for-a-given-avs-address.mdx). These routes allow developers to track the current operators and stakers associated with a specific AVS address. We are excited to see how the new endpoints will be utilized in the various AVS applications.
